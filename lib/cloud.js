...
function parseAllstar(url) {
  ...
  return {
    url: `https://allstar.gg/clip/${clipId}`,
    clipId,
    embed: `https://allstar.gg/iframe?clip=${clipId}`
  };
}
...
function mapCsrep(stats) {
  const saved = stats && stats.extra && stats.extra.csrep;
...
function mapClip(row, playerName = "") {
  return {
    ...
    url: row.allstar_url,
    clipId: row.clip_id,
    embed: `https://allstar.gg/iframe?clip=${row.clip_id}`,
    map: row.map || "",
...
async function getPlayer(id) {
  ...
  const clips = await rest(`clips?player_id=eq.${encodeURIComponent(id)}&select=*&order=created_at.desc`);
  return {
    ...mapPlayer(player, Array.isArray(stats) ? stats[0] : null),
...
// Guarda a resposta do CSRep em player_stats.extra.csrep, sem tocar nos
// números que o painel preencheu à mão.
async function saveCsrep(playerId, payload) {
  ...
  const extra = { ...(current.extra || {}), csrep: payload };
  await rest("player_stats?on_conflict=player_id", {
    method: "POST",
...
async function listClips() {
  const clips = await rest("clips?select=*&order=created_at.desc");
