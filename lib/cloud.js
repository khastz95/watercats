...
async function createJoinRequest(body = {}) {
  ...
  if (clip(body.website, 120) || clip(body.company, 120)) {
  ...
  const discord = clip(body.discord, 80);
  const steamId = clip(body.steamId, 32).replace(/\D/g, "");
  const steamUrl = clip(body.steamUrl, 200);
  const era = clip(body.era, 24);
  const role = clip(body.role, 60);
  const message = clip(body.message, 1200);
  const contact = clip(body.contact, 120);

  if (name.length < 2) {
    const err = new Error("Nome completo é obrigatório");
    err.status = 400;
    throw err;
  }
  if (nick.length < 2) {
    const err = new Error("Nick é obrigatório");
    err.status = 400;
    throw err;
  }
  if (!discord && !steamId && !steamUrl && !contact) {
    const err = new Error("Deixe Discord, Steam ou um contato");
    err.status = 400;
    throw err;
  }

  const allowedEra = new Set(["1.6", "source", "go", "cs2", "more", ""]);
  if (!allowedEra.has(era)) {
    const err = new Error("Era inválida");
    err.status = 400;
    throw err;
  }
  ...
  const recent = await rest(
    `join_requests?created_at=gte.${encodeURIComponent(since)}&select=nick,discord,steam_id&limit=40`
  );
  const nickKey = nick.toLowerCase();
  const hit = (recent || []).some((row) => {
    if (String(row.nick || "").toLowerCase() === nickKey) return true;
    if (discord && row.discord === discord) return true;
    if (steamId && row.steam_id === steamId) return true;
    return false;
  });
  if (hit) {
    const err = new Error("Já recebemos um pedido parecido há pouco. Espera um pouco e tenta de novo.");
    err.status = 429;
    throw err;
  }

  const rows = await rest("join_requests", {
    method: "POST",
    body: JSON.stringify({
      name,
      nick,
  ...
  return mapJoinRequest(Array.isArray(rows) ? rows[0] : rows);
}

async function listJoinRequests(status = "") {
  const filter = status
    ? `status=eq.${encodeURIComponent(status)}&`
    : "";
  const rows = await rest(
    `join_requests?${filter}order=created_at.desc&select=*`
  );
  return (rows || []).map(mapJoinRequest);
}

async function updateJoinRequest(id, body = {}) {
  ...
  if (body.notes != null) patch.notes = clip(body.notes, 800);
  const rows = await rest(`join_requests?id=eq.${encodeURIComponent(id)}`, {
    method: "PATCH",
...
async function deleteJoinRequest(id) {
  await rest(`join_requests?id=eq.${encodeURIComponent(id)}`, { method: "DELETE" });
}

function mapMarcaSignup(row) {
  if (!row) return null;
  return {
    id: row.id,
    email: row.email || "",
    name: row.name || "",
    source: row.source || "marca",
    createdAt: row.created_at || null
  };
}

async function createMarcaSignup(body = {}) {
  if (clip(body.website, 120) || clip(body.company, 120)) {
    return { ok: true, ignored: true };
  }
  ...
  const name = clip(body.name, 80);
  const source = clip(body.source, 40) || "marca";
  ...
  const existing = await rest(
    `marca_waitlist?email=eq.${encodeURIComponent(email)}&select=id,email,name,source,created_at&limit=1`
  );
  if (Array.isArray(existing) && existing[0]) {
    return mapMarcaSignup(existing[0]);
  }

  try {
    const rows = await rest("marca_waitlist", {
      method: "POST",
      body: JSON.stringify({ email, name, source })
    });
    return mapMarcaSignup(Array.isArray(rows) ? rows[0] : rows);
  } catch (err) {
    // Corrida rara: outro POST com o mesmo e-mail entre o SELECT e o INSERT.
    if (err.status === 409 || /duplicate|unique/i.test(String(err.message || ""))) {
      const again = await rest(
        `marca_waitlist?email=eq.${encodeURIComponent(email)}&select=id,email,name,source,created_at&limit=1`
      );
      if (Array.isArray(again) && again[0]) return mapMarcaSignup(again[0]);
    }
    throw err;
  }
}
